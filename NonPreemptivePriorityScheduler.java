...
public class NonPreemptivePriorityScheduler {
    ...
    // Method to perform scheduling based on Non-Preemptive Priority Scheduling algorithm.
    public void schedule(List<Process> processes) {
        int currentTime = 0; // Initialize the current time to zero.

        // Priority queue to select the process with the highest priority and shortest burst time.
        Queue<Process> readyQueue = new PriorityQueue<>(
                Comparator.comparingInt((Process p) -> p.priority) // Compare by priority.
                        .thenComparingInt(p -> p.burstTime) // If priorities are equal, compare by burst time.
                        .thenComparingInt(p -> p.arrivalTime) // If burst times are also equal, compare by arrival time.
        );
        ...
        while (!allProcessesHandled) { // Loop until all processes are handled.
            ...
            if (!readyQueue.isEmpty()) { // If the ready queue has processes.
                ...
                // Select the process with the highest priority and shortest burst time.
